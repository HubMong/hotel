          ...
          <tr>
            ...
            <th>
              ...
              <button class="sort-btn" @click.stop="sortBy('name')" style="background:none;border:none;padding:0;margin-left:4px;vertical-align:middle;">
                <svg width="18" height="23" viewBox="0 0 12 16" xmlns="http://www.w3.org/2000/svg" style="display:inline-block;vertical-align:middle;">
                  <polygon points="6,2 2,6 10,6" :fill="sort.prop==='name' && sort.order==='ascending' ? '#222' : '#ccc'" />
                  <polygon points="6,14 2,10 10,10" :fill="sort.prop==='name' && sort.order==='descending' ? '#222' : '#ccc'" />
                </svg>
              </button>
            </th>
            <th>
          ...
          <tr v-for="business in businesses" :key="business.id" :class="getRowClass(business.status)">
            ...
            <td>
              <div class="business-info">
                ...
                <small class="business-user">{{ business.businessEmail || '이메일 미제공' }}</small>
              </div>
            </td>
        ...
        <div class="modal-body">
          <div class="detail-section">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item">
                <label>사용자 이메일:</label>
                <span>{{ selectedBusiness.businessEmail || '이메일 미제공' }}</span>
              </div>
              <div class="detail-item">
                <label>연락처:</label>
...
export default {
  ...
  const allowed = new Set(['id','status','name','businessName','businessNumber','businessPhone','address','createdAt'])
    ...
    async updateStatus(business, newStatus) {
      ...
      try {
        ...
        // 대시보드/사용자 목록 갱신 신호 전파 (역할 변경 반영)
        try { window.dispatchEvent(new CustomEvent('admin:refresh-dashboard', { detail: { source: 'hotel-management', action: 'status-changed', status: newStatus, hotelId: business.id } })) } catch {}
        try { window.dispatchEvent(new CustomEvent('admin:refresh-users', { detail: { source: 'hotel-management', action: 'status-changed', status: newStatus, hotelId: business.id } })) } catch {}
