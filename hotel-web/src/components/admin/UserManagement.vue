  ...
  <table v-else class="user-table">
          ...
          <tbody>
            <tr v-for="row in users" :key="row.id" class="user-row" :class="{ active: selectedUser?.id === row.id }" @click="selectUser(row)">
              ...
              <td class="user-status">
                ...
                <div class="lock-info" v-if="isLoginLocked(row)">
                  <span class="lock-chip">
                    {{ lockTypeLabel(row.loginLockType) }} · {{ formatLockRemaining(row) }}
                  </span>
                </div>
              </td>
              <td class="user-join">
      ...
      <div v-if="selectedUser && !showAssignCouponModal" class="user-summary-panel">
        ...
        <div class="panel-content">
          <div v-if="isLoginLocked(selectedUser)" class="lock-alert-panel">
            <div class="lock-alert-badge">{{ lockTypeLabel(selectedUser.loginLockType) }}</div>
            <div class="lock-alert-text">
              {{ formatLockRemaining(selectedUser) }}
              <span v-if="selectedUser.loginLockUntil" class="lock-alert-until">
                · {{ formatDate(selectedUser.loginLockUntil) }} 해제 예정
              </span>
            </div>
          </div>
          ...
          <div class="summary-section">
            ...
            <div class="panel-actions">
              ...
              <button
                v-if="isLoginLocked(selectedUser)"
                class="panel-action-btn warning"
                @click="unlockLogin(selectedUser)"
              >
                로그인 잠금 해제
              </button>
            </div>
...
export default {
  ...
  if (this.advancedFilters.joinPeriod) params.joinPeriod = this.advancedFilters.joinPeriod;
        ...
        this.users = content.map(u => {
          const remaining = u.lockRemainingMinutes != null ? Number(u.lockRemainingMinutes) : null;
          const normalizedRemaining = remaining != null && remaining > 0 ? Math.ceil(remaining) : 0;
          return {
            ...u,
            lockRemainingMinutes: normalizedRemaining,
            loginLockType: u.loginLockType || null,
            loginLockUntil: u.loginLockUntil || null
          };
        });
    ...
    isLoginLocked(user) {
      ...
      const remaining = Number(user.lockRemainingMinutes);
      return Number.isFinite(remaining) && remaining > 0;
    },

    lockTypeLabel(type) {
      const map = { SHORT: '임시 잠금', LONG: '장기 잠금' };
      return map[type] || '잠금';
    },

    formatLockRemaining(user) {
      if (!this.isLoginLocked(user)) return '';
      const minutes = Math.max(0, Math.ceil(Number(user.lockRemainingMinutes)));
      const hours = Math.floor(minutes / 60);
      const mins = minutes % 60;
      if (hours > 0 && mins > 0) return `${hours}시간 ${mins}분 남음`;
    ...
    async unlockLogin(user) {
      if (!user) return;
      if (!confirm(`${user.name}님의 로그인 잠금을 해제하시겠습니까?`)) return;
      try {
        await api.post(`/admin/users/${user.id}/unlock-login`);

        user.lockRemainingMinutes = 0;
        user.loginLockType = null;
        ...
        if (index !== -1) {
          this.users.splice(index, 1, cleared);
        }

        if (this.selectedUser && this.selectedUser.id === user.id) {
          this.selectedUser = cleared;
        }

        try {
          window.dispatchEvent(new CustomEvent('admin:refresh-users', { detail: { reason: 'login-unlock', userId: user.id } }));
        } catch {}

        alert('로그인 잠금이 해제되었습니다.');
      } catch (error) {
        alert(error?.response?.data?.message || '로그인 잠금 해제에 실패했습니다.');
      }
    }
  }
}
