...
export default {
    ...
    return {
      ...
      dateRangeConfig: {
        ...
        onReady: (selected, _dateStr, instance) => {
          console.log('[SearchForm FP onReady] selected =', this.asArray(selected).map(this.toYmd));
        ...
        onChange: (selectedDates) => {
          console.log('[SearchForm FP onChange] selected =', this.asArray(selectedDates).map(this.toYmd));
        },

        onValueUpdate: (selectedDates) => {
          console.log('[SearchForm FP onValueUpdate] =', this.asArray(selectedDates).map(this.toYmd));
        },
  ...
  computed: {
    travelerLabel() {
      ...
      // children=0도 항상 표시하고 싶으면 아래 조건 제거
  ...
  methods: {
    ...
    // ✅ 검색: children=0도 항상 쿼리에 포함되도록 수정
    goSearch() { // [CHANGED]
      if (!this.destination) {
        alert("목적지를 입력해주세요.");
        return;
   ...
   adults: (this.adults ?? 1),        // 최소 1
  children: (this.children ?? 0),    // 0도 포함
 };
