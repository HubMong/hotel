...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
	...
	@Query("SELECT r.roomId, COALESCE(SUM(r.numRooms), 0) FROM Reservation r WHERE r.roomId in :roomIds AND r.status = 'COMPLETED' AND r.startDate <= :searchEnd AND r.endDate >= :searchStart GROUP BY r.roomId")
	List<Object[]> sumNumRoomsOverlappingPeriod(@Param("roomIds") List<Long> roomIds, @Param("searchStart") Instant searchStart, @Param("searchEnd") Instant searchEnd);
}
