// src/main/java/com/example/backend/admin/service/AdminHotelService.java
...
public class AdminHotelService {
    ...
    public Page<HotelAdminDto> list(String name, Integer minStar, Hotel.ApprovalStatus status, Pageable pageable) {
        ...
        if (status != null) {
            ...
            // ✅ owner 즉시 로딩 버전 사용 (@EntityGraph on findAll(Pageable))
    ...
    private Comparator<HotelAdminDto> comparatorFor(Sort.Order order) {
        ...
        // 정렬 허용 필드만 한정
        switch (prop) {
            case "name" -> base = Comparator.comparing(HotelAdminDto::name, Comparator.nullsLast(Comparator.naturalOrder()));
    ...
    public HotelAdminDto get(Long id) {
        ...
        // LAZY 초기화
        rooms.forEach(r -> r.getImages().size());

        return HotelAdminDto.from(
            hotel,
            hotel.getBusinessName(),
    ...
    public void approve(Long id, Long adminUserId, String note) {
        ...
        // ✅ 승인 시 거부사유는 비움(메모를 남길거면 별도 감사 로그 권장)
    ...
    public void reject(Long id, String reason) {
        ...
        // 선택: 거부는 승인일시를 비워두는 편이 일반적
        hotel.setApprovalDate(null);
    ...
    private HotelAdminDto mapToAggregatedDto(Object[] row) {
        ...
        if (hotelId == null) return fallbackDto(row);
    ...
    private HotelAdminDto fallbackDto(Object[] row) {
        ...
        if (hotelId == null) throw new IllegalArgumentException("호텔 정보를 확인할 수 없습니다.");
        Hotel hotel = hotelRepository.findById(hotelId)
            .orElseThrow(() -> new IllegalArgumentException("호텔을 찾을 수 없습니다: " + hotelId));
    ...
    private String businessNumberOf(Hotel hotel) {
        return (hotel != null && hotel.getBusinessId() != null) ? String.valueOf(hotel.getBusinessId()) : null;
    }

    private int intValue(Object[] row, int index) { return intValue(row[index]); }
    private int intValue(Object value) {
        if (value instanceof Number n) return n.intValue();
        if (value instanceof String s && !s.isBlank()) { try { return (int) Double.parseDouble(s); } catch (NumberFormatException ignored) {} }
        return 0;
    }
    private long longValue(Object[] row, int index) { return longValue(row[index], 0L); }
    private Long longValue(Object[] row, int index, Long def) { return longValue(row[index], def); }
    private Long longValue(Object value, Long def) {
        if (value instanceof Number n) return n.longValue();
        if (value instanceof String s && !s.isBlank()) { try { return Math.round(Double.parseDouble(s)); } catch (NumberFormatException ignored) {} }
        return def;
    }
    private double doubleValue(Object[] row, int index) { return doubleValue(row[index]); }
    private double doubleValue(Object value) {
        if (value instanceof Number n) return n.doubleValue();
        if (value instanceof String s && !s.isBlank()) { try { return Double.parseDouble(s); } catch (NumberFormatException ignored) {} }
        return 0.0;
    }
