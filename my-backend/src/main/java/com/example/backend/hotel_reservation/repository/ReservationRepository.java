...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("""
           ...
           WHERE r.roomId IN (
             ...
             AND r.status = com.example.backend.hotel_reservation.domain.Reservation$Status.COMPLETED
           """)
    ...
    // ★ 남은 객실 계산용 오버랩 합산:
    //   COMPLETED + (만료 전 PENDING) 포함, 날짜는 [start, end) 규칙
    @Query("""
        SELECT r.roomId AS roomId, COALESCE(SUM(r.numRooms),0) AS usedQty
        FROM Reservation r
        WHERE r.roomId IN :roomIds
          AND r.startDate < :endDate
          AND r.endDate   > :startDate
          AND (
                r.status = com.example.backend.hotel_reservation.domain.Reservation$Status.COMPLETED
                OR (r.status = com.example.backend.hotel_reservation.domain.Reservation$Status.PENDING AND r.expiresAt > :now)
              )
        GROUP BY r.roomId
    """)
    List<Object[]> sumNumRoomsOverlappingPeriod(
